...
const campaignPerformance_1 = require("../campaignPerformance");
function registerQueryCampaignPerformanceTool(server) {
    server.registerTool("queryCampaignPerformance", {
        ...
        try {
            const client = await (0, campaignPerformance_1.getPostgresClient)();
            const filters = await (0, campaignPerformance_1.aiDeriveFilters)(sanitizedQuestion);
            const safeLimit = 3;
            const campaigns = await (0, campaignPerformance_1.fetchCampaignRows)(client, filters, safeLimit);
            ...
            const snapshots = await (0, campaignPerformance_1.fetchPromptAdsSnapshots)(client, campaignIds);
            const channelMap = await (0, campaignPerformance_1.fetchCampaignChannels)(client, campaignIds);
            const salesMap = await (0, campaignPerformance_1.fetchSalesSummaries)(client, campaignIds);
            const { summaryMap, statusMap } = await (0, campaignPerformance_1.fetchCrmInsights)(client, campaignIds);
            ...
            if (highlighted) {
                summaryParts.push(`La campaña ${highlighted.campaignName ?? highlighted.campaignId} reportó alcance ${(0, campaignPerformance_1.formatLargeNumber)(highlighted.reach)}, tasa de éxito ${(0, campaignPerformance_1.formatPercent)(highlighted.conversionRate)}, ventas estimadas ${(0, campaignPerformance_1.formatLargeNumber)(highlighted.salesAmount)}.`);
            }
            const output = {
                question: sanitizedQuestion,
