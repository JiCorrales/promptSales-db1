...
async function toolGetContent({ descripcion }) {
    let images = [];
    try {
...
async function toolSearchMusic({ query, limit = 5 }) {
    const tracks = await (0, spotify_1.searchTrack)(query, limit);
    return { tracks };
}
async function toolCreateCampaign({ descripcion, publico, duracion = "1 mes", presupuesto = 5000 }) {
