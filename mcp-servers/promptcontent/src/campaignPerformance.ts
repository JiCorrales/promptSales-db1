...
export async function aiDeriveFilters(question: string): Promise<{ campaignId: number | null; companyId: number | null }> {
    const key = process.env.OPENAI_API_KEY
    if (!key) return { campaignId: null, companyId: null }
    ...
    const system = `Devuelve SOLO un JSON vA­lido con esta forma exacta:
{
  ...
  "companyId": <number|null>
    ...
    try {
        ...
        const responseStatus: "ok" | "partial" = parsed ? "ok" : "partial"
...
export async function fetchCampaignRows(client: Client, filters: { campaignId?: number | null; companyId?: number | null }, limit: number) {
    ...
    return rows
        ...
        .filter((entry): entry is CampaignRow => entry !== null)
}
...
export async function fetchCrmInsights(client: Client, campaignIds: number[]) {
    if (campaignIds.length === 0) {
        return { summaryMap: new Map<number, any>(), statusMap: new Map<number, Record<string, number>>() }
    }
    const summaryResult: QueryResult<any> = await client.query(
        `
        SELECT
            campaign_id,
    ...
    const statusResult: QueryResult<any> = await client.query(
        `
        SELECT
            campaign_id,
